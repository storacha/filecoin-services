...
func runApproveService(cobraCmd *cobra.Command, args []string) error {
	...
	// Additional validation for deposit-specific requirements
	if includeDeposit && depositAmount == "" {
	...
	// Create signer manager and load payer's private key
	...
	if includeDeposit {
		...
		permitSig, err := GeneratePermitSignature(
			ctx,
			client,
			cfg.TokenAddr(),
		...
		tx, err = paymentsContract.DepositWithPermitAndApproveOperator(
			...
			permitSig.R,
			permitSig.S,
			cfg.ServiceAddr(), // Operator is the FilecoinWarmStorageService contract
			rateAllowanceBig,
			lockupAllowanceBig,
