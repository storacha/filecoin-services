...
// ParseDollarAmount parses a dollar amount string (e.g., "10", "$10", "10.50") to a float64
func ParseDollarAmount(amountStr string) (float64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("invalid dollar amount format: %s (expected format: 10, $10, 10.50): %w", amountStr, err)
	}
...
// DollarToBaseUnits converts a dollar amount to base token units
// decimals: the number of decimals the token uses (e.g., 6 for USDC, 18 for standard ERC20)
// For USDFC with 6 decimals: $1.00 = 1,000,000 base units
// For standard ERC20 with 18 decimals: $1.00 = 1,000,000,000,000,000,000 base units
func DollarToBaseUnits(dollars float64, decimals uint8) (*big.Int, error) {
	if dollars <= 0 {
		return nil, fmt.Errorf("amount must be greater than 0")
	}

	// Calculate base units: dollars * 10^decimals
	// Using big.Float for precision
	multiplier := new(big.Int).Exp(big.NewInt(10), big.NewInt(int64(decimals)), nil)

	// Convert dollars to big.Float and multiply
	dollarsBig := new(big.Float).SetFloat64(dollars)
	multiplierFloat := new(big.Float).SetInt(multiplier)
	result := new(big.Float).Mul(dollarsBig, multiplierFloat)

	// Convert to big.Int (truncates decimals)
	baseUnits, _ := result.Int(nil)

	if baseUnits.Sign() <= 0 {
		return nil, fmt.Errorf("amount too small to represent in base units")
	}
