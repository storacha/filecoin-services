...
func QueryRailsForPayee(ctx context.Context, rpcURL string, paymentsAddress common.Address, payee common.Address, token common.Address) ([]bindings.PaymentsRailInfo, error) {
	...
	// Query rails for payee with pagination (get first 100 rails)
	offset := big.NewInt(0)
	limit := big.NewInt(100)
	result, err := payments.GetRailsForPayeeAndToken(&bind.CallOpts{Context: ctx}, payee, token, offset, limit)
	if err != nil {
		return nil, fmt.Errorf("getting rails for payee: %w", err)
	}

	// Return just the Results array from the struct
