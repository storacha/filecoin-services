...
SERVICE_PAYMENTS_IMPLEMENTATION_ADDRESS=$(forge create --password "$PASSWORD" --broadcast --nonce $NONCE \
  ...
  src/FilecoinWarmStorageService.sol:FilecoinWarmStorageService --constructor-args $PDP_VERIFIER_PROXY_ADDRESS $FILECOIN_PAY_ADDRESS $USDFC_TOKEN_ADDRESS $FILBEAM_BENEFICIARY_ADDRESS $SERVICE_PROVIDER_REGISTRY_PROXY_ADDRESS $SESSION_KEY_REGISTRY_ADDRESS | grep "Deployed to" | awk '{print $3}')
...
FWSS_PROXY_ADDRESS=$(forge create --password "$PASSWORD" --broadcast --nonce $NONCE lib/pdp/src/ERC1967Proxy.sol:MyERC1967Proxy --constructor-args $SERVICE_PAYMENTS_IMPLEMENTATION_ADDRESS $INIT_DATA | grep "Deployed to" | awk '{print $3}')
if [ -z "$FWSS_PROXY_ADDRESS" ]; then
  echo "Error: Failed to extract FilecoinWarmStorageService proxy address"
  exit 1
fi
echo "FilecoinWarmStorageService proxy deployed at: $FWSS_PROXY_ADDRESS"
...
echo "FilecoinPayV1 contract address: $FILECOIN_PAY_ADDRESS"
...
if [ -n "$SERVICE_PAYMENTS_IMPLEMENTATION_ADDRESS" ]; then
    update_deployment_address "$CHAIN" "FWSS_IMPLEMENTATION_ADDRESS" "$SERVICE_PAYMENTS_IMPLEMENTATION_ADDRESS"
fi
if [ -n "$FWSS_PROXY_ADDRESS" ]; then
    update_deployment_address "$CHAIN" "FWSS_PROXY_ADDRESS" "$FWSS_PROXY_ADDRESS"
fi
...
if [ -n "$SERVICE_PAYMENTS_IMPLEMENTATION_ADDRESS" ] || [ -n "$FWSS_PROXY_ADDRESS" ]; then
